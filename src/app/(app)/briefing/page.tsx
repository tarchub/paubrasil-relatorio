...
export default async function BriefingPage() {
  ...
  const topOpp = b.top_opportunity as Item | null;
    ...
    <div className="space-y-6">
      ...
      {b.intro_pt && (
        <div className="card border-gold/20 bg-gradient-to-b from-gold/[0.06] to-transparent p-6">
          <p className="font-display text-lg leading-relaxed text-sand">{b.intro_pt}</p>
        </div>
      )}

      <div className="grid gap-4 lg:grid-cols-2">
        <Block title="Principais novidades" items={asItems(b.top_news)} />
        <Block title="Tendências em crescimento" items={asItems(b.trends)} />
        <Block title="SaaS encontrados" items={asItems(b.saas)} />
        <Block title="Estratégias para testar" items={asItems(b.strategies)} />
      </div>
